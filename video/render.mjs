...
try {
  ...
  const audioEvents = await page.evaluate((rate) => window.filmAudioEvents.map(event => ({ ...event, at: (event.at - window.filmAudioStart) / rate / 1000 })), CAPTURE_RATE)
...
for (let index = 0; index < captured.length; index++) {
  ...
  concat.push(`file '${filename}'`, 'option framerate 1000', `duration ${Math.max(0.001, (end - start) / CAPTURE_RATE)}`)
}
...
console.log(file)
const gaps = captured.slice(1).map((f, i) => (f.time - captured[i].time) / CAPTURE_RATE).filter((_,i)=>captured[i].time < startedAt + 8 * CAPTURE_RATE)
console.log(JSON.stringify({theme,frames:captured.length,maxGap:Math.max(...gaps),over50ms:gaps.filter(g=>g>.05).length}))
